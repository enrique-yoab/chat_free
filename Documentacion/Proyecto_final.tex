...
\newpage
\begin{itemize}
    \item \textbf{Mario Mendoza González}
\end{itemize}
El desarrollo de este proyecto permitió entender de una forma mucho más práctica cómo funcionan varios de los conceptos vistos en la materia cuando se integran en una sola aplicación. Aunque al inicio temas como la comunicación entre procesos, el uso de hilos o la programación con sockets parecían independientes, durante la implementación fue posible ver cómo todos ellos se relacionan para resolver un mismo problema.

Uno de los mayores retos fue lograr que la interfaz gráfica, la comunicación mediante tuberías y la conexión P2P trabajaran al mismo tiempo sin interferirse entre sí. Esto ayudó a comprender la importancia de la concurrencia y de una buena organización del sistema, ya que cada proceso y cada hilo tienen una función específica dentro de la aplicación.

Además del aspecto técnico, el proyecto permitió acercarse al desarrollo de software de una manera más completa, considerando no solo que el programa funcionara, sino también que fuera estable, fácil de utilizar y capaz de mantener la información de las conversaciones. Entender cómo un mensaje viaja desde la interfaz, pasa por los mecanismos de comunicación interna y finalmente llega a otro nodo hizo más claro el funcionamiento de un sistema distribuido real (una aplicación tipo messenger).

Finalmente, quedó claro que detrás de las aplicaciones de mensajes existe una gran cantidad de procesos trabajando. La experiencia obtenida durante el desarrollo ayudó a reforzar los conocimientos de la materia y a comprender mejor lo que implica diseñar sistemas.

\section{\large 6. Referencias}
\begin{itemize}
